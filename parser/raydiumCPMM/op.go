...
type RaydiumCPMMDex struct {
	model.DexRouter
	Auth *string
}

func (r *RaydiumCPMMDex) ParseLiquidityCreate(accounts []*solana.AccountMeta, data []byte, extra ...interface{}) (*model.ArchDexMod, error) {
...
func (r *RaydiumCPMMDex) ParseLiquidityRemove(accounts []*solana.AccountMeta, data []byte, extra ...interface{}) (*model.ArchDexMod, error) {
	...
	decodedData.CoinAddr = accounts[10].PublicKey.String()
	decodedData.PcAddr = accounts[11].PublicKey.String()
	decodedData.CoinAccount = accounts[6].PublicKey.String()
	decodedData.PcAccount = accounts[7].PublicKey.String()
	if allInner != nil {
...
func (r *RaydiumCPMMDex) ParseLiquidityAdd(accounts []*solana.AccountMeta, data []byte, extra ...interface{}) (*model.ArchDexMod, error) {
	allInner, ok := extra[0].(*[]solana.CompiledInstruction)
	accountProgramKeysMeta, ok := extra[1].(solana.AccountMetaSlice)
...
func (r *RaydiumCPMMDex) ParseLiquiditySwap(accounts []*solana.AccountMeta, data []byte, extra ...interface{}) (*model.ArchDexMod, error) {
	if len(extra) < 2 {
	...
	lpStr := accounts[3].PublicKey.String()
	auth := accounts[0].PublicKey.String()
	//没有pcAccount    先用InAccount代替
	//对于用户的操作，链上字段Input Vault 为用户输出的代币
	//对于用户的操作，链上字段Output Vault 为用户输入的代币
	InAccount := accounts[7].PublicKey.String()
	OutAccount := accounts[6].PublicKey.String()
	pcAccount := InAccount
	coinAccount := OutAccount
	inAddr := accounts[11].PublicKey.String()
	outAddr := accounts[10].PublicKey.String()
	...
	decodedData.InAddr = &inAddr
	decodedData.OutAddr = &outAddr
	decodedData.CoinAddr = &inAddr
	decodedData.PcAddr = &outAddr
...
func (r *RaydiumCPMMDex) UniCall(accounts []*solana.AccountMeta, data []byte, extra ...interface{}) (*model.ArchDexMod, error) {
	defer func() {
		if r := recover(); r != nil {
